...
trait InsertValues[VExpr, VCol <: VExpr, VSc, R]
    extends Returning.InsertBase[VCol]
    with Query.ExecuteUpdate[Int] {
  def skipColumns(x: (VCol => Column[?])*): InsertValues[VExpr, VCol, VSc, R]
}
object InsertValues {
  class Impl[VExpr, VCol <: VExpr, VSc, R](
      insert: Insert[VExpr, VCol, VSc, R],
      values: Seq[R],
      dialect: DialectTypeMappers,
      qr: Queryable.Row[VCol, R],
      skippedColumns: Seq[Column[?]]
  ) extends InsertValues[VExpr, VCol, VSc, R] {
    ...
    override def skipColumns(x: (VCol => Column[?])*): InsertValues[VExpr, VCol, VSc, R] = {
